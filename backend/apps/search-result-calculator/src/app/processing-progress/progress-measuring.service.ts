...
export class ProgressMeasuringService {
  ...
  public async setProgress(searchId: string): Promise<void> {
    ...
    if (foundSummary) {
      const summarized = await this.determineProgress(searchId, foundSummary.expectedNumberOfParts)
      if (summarized) {
  ...
  public async summarizeProgress(searchId: string, expectedNumberOfParts: number): Promise<void> {
    const created = await this.processingProgressRepository.create({
      searchId,
    ...
    const summarized = await this.determineProgress(created.searchId, created.expectedNumberOfParts)
    if (summarized) {
      logger.debug(`All hotels parts for search id [${searchId}] were processed and the progress could be summarized`)
    }
  }

  private async determineProgress(searchId: string, expectedNumberOfParts: number): Promise<boolean> {
    const partsCount = await this.processingProgressRepository.getQuantityOfType(searchId, ProcessingProgressType.PART)
    const eligibleToSummarize = partsCount === expectedNumberOfParts
    if (eligibleToSummarize) {
      ...
      this.userNotificationSender.notifyAboutHotelsProcessingFinished(searchId)
    } else {
