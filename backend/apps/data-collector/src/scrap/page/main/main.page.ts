...
export class MainPage {
  ...
  public async extractSearchPlaceIdentifierFromFormInputs(searchPlace: string): Promise<SearchPlaceIdentifier> {
    ...
    const placeIdLat = await this.browserService.getValueFromInput(MainPageElement.PLACE_ID_LAT_INPUT.selector);
    const placeIdLon = await this.browserService.getValueFromInput(MainPageElement.PLACE_ID_LON_INPUT.selector);
    return {
      destination,
      destId,
      destType,
